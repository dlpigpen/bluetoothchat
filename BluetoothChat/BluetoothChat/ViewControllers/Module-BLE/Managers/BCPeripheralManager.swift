...
extension BCPeripheralManager {
    ...
    func sendMessage(message: String, roomID: String) {
        ...
        let messageObject = BCMessage(message: message, name: name, isSelf: true, isStatus: false, timestamp: timestamp, isRead: false,  peripheralID: NSUUID(UUIDString: roomID))
    ...
    func sendName(isSelf isSelf: Bool = false, oldName: String? = nil) {
        guard var name = BCDefaults.stringForKey(.Name) else {
            UIApplication.presentAlert(title: "Name Not Set", message: "Please enter your name by clicking the information icon in the top right corner")
            return
        }
        ...
        if let oldName = oldName where isSelf {
            ...
            let chatModel = ChattingModel()
            let listRoom: NSMutableArray = roomModel.getAllRoom()
            for room in listRoom {
                if let myroom = room as? RoomObject {
                     let roomID = myroom.uIIDAnother
                      message.timestamp = timestamp
                      message.peripheralID = NSUUID(UUIDString: roomID)
                    chatModel.createAChatting(message)
                }
            }
            delegate?.updateWithNewMessage(message, isStatus: true)
        }
...
extension BCPeripheralManager: CBPeripheralManagerDelegate {
    ...
    func peripheralManagerDidUpdateState(peripheral: CBPeripheralManager) {
        ...
        timer = NSTimer.scheduledTimerWithTimeInterval(5, target: self, selector: "startAdvertisingFromTimer:", userInfo: 3, repeats: true)
    }
