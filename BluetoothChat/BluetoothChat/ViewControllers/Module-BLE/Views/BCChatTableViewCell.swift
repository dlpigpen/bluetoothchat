...
extension BCChatTableViewCell {
    ...
    private func styledMetaDataLabel(message: BCMessage) -> UILabel {
        ...
        let metaDataDate = message.timestamp
        
        let array = message.name.componentsSeparatedByString(",")
        let name =  array.first!
        let metaDataAttributedString = NSMutableAttributedString(string: "\(name)  \(metaDataDate)")

        // Find name and time ranges
        let nameRange = (metaDataAttributedString.string as NSString).rangeOfString(name)
